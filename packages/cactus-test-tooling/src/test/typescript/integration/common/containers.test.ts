...
describe(testCase, () => {
  ...
  test(testCase, async () => {
    // Use mkdtempSync to create a private, uniquely-owned temp directory,
    // avoiding the insecure pattern of writing files directly to os.tmpdir()
    // where the filename could be predicted and the file hijacked.
    const srcFileDir = fs.mkdtempSync(path.join(os.tmpdir(), "cacti-test-"));
