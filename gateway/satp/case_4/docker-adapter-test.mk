...
# Using André Augusto's image temporarily - official image TBD
# See: https://github.com/hyperledger-cacti/cacti/issues/3932
# =============================================================================
GATEWAY_IMAGE := ghcr.io/hyperledger-cacti/satp-hermes-gateway
...
build:
	...
	docker pull ghcr.io/hyperledger-cacti/satp-hermes-gateway:latest
	docker tag ghcr.io/hyperledger-cacti/satp-hermes-gateway:latest $(IMAGE_NAME):$(IMAGE_TAG)
	docker tag ghcr.io/hyperledger-cacti/satp-hermes-gateway:latest $(IMAGE_NAME):latest
	@echo "✓ Image ready: $(IMAGE_NAME):$(IMAGE_TAG)"

## Build without cache (forces fresh pull)
build-no-cache:
	@echo "Pulling latest Docker image (no cache): $(IMAGE_NAME):$(IMAGE_TAG)..."
	docker pull ghcr.io/hyperledger-cacti/satp-hermes-gateway:latest
	docker tag ghcr.io/hyperledger-cacti/satp-hermes-gateway:latest $(IMAGE_NAME):$(IMAGE_TAG)
	docker tag ghcr.io/hyperledger-cacti/satp-hermes-gateway:latest $(IMAGE_NAME):latest
...
run: stop
	...
	docker run -d --rm \
		...
		-p $(CLIENT_PORT):3011 \
		-p $(API_PORT):4010 \
		-v "$$(pwd)/$(GATEWAY_CONFIG):/opt/cacti/satp-hermes/config/config.json:ro" \
		-v "$$(pwd)/$(ADAPTER_CONFIG):/opt/cacti/satp-hermes/config/adapter-config.yml:ro" \
		$(IMAGE_NAME):latest
