...
async function main(port) {
  ...
  if (process.argv[2] !== undefined && typeof Number(process.argv[2]) === 'number') {
    if (Number(process.argv[2]) === 1) {
      ...
      const satpTokenContract = await SATPNonFungibleTokenContractFactory.deploy(deployerAddress);
      await satpTokenContract.waitForDeployment();
      console.log(`${port} - SATPNonFungibleTokenContract deployed to:`, satpTokenContract.target);
      ...
      const giveRole2Tx = await satpTokenContract.connect(deployer).grantBridgeRole(BRIDGE_ADDRESS);
      await giveRole2Tx.wait();

      if (port === 8545) {
        // Mint tokens to the user address in the source chain (8545)
        console.log(`${port} - Minting tokens...`);
        const mintTx = await satpTokenContract.connect(deployer).mint(userAddress, 1001);
        await mintTx.wait();
        ...
        const approve2Tx = await satpTokenContract.connect(user).approve(BRIDGE_ADDRESS, 1001);
        ...
        const allowance = await satpTokenContract.hasPermission(
          BRIDGE_ADDRESS,
          1001
        );
      ...
      if (port === 8546) {
        ...
        console.log(`${port} - Approving bridge address ${BRIDGE_ADDRESS}...`);
        const approve2Tx = await deployedSatpTokenContract.connect(user).approve(BRIDGE_ADDRESS, 1001);
        await approve2Tx.wait();
        
        // Check allowance of bridge address given by user in the source chain (8546)
        console.log(`${port} - Checking allowance...`);
        const allowance = await deployedSatpTokenContract.hasPermission(
          BRIDGE_ADDRESS,
          1001
        );
        console.log(`${port} - Allowance:`, allowance.toString());
