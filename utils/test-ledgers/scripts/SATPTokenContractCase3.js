...
const SATP_TOKEN_BYTECODE =
  require("../artifacts/contracts/SATPTokenContract.sol/SATPTokenContract.json")[
    "bytecode"
  ];
const SATP_TOKEN_ABI =
  require("../artifacts/contracts/SATPTokenContract.sol/SATPTokenContract.json")[
    "abi"
  ];
...
async function main(port) {
  ...
  const deployer = await provider.getSigner(
    accounts[START_ADDRESS_INDEX].address,
  );
  const user = await provider.getSigner(
    accounts[START_ADDRESS_INDEX + 1].address,
  );

  if (
    process.argv[2] !== undefined &&
    typeof Number(process.argv[2]) === "number"
  ) {
    if (Number(process.argv[2]) === 1) {
      ...
      const SATPTokenContractFactory = new ethers.ContractFactory(
        SATP_TOKEN_ABI,
        SATP_TOKEN_BYTECODE,
        deployer,
      );
      const satpTokenContract =
        await SATPTokenContractFactory.deploy(deployerAddress);
      await satpTokenContract.waitForDeployment();
      console.log(
        `${port} - SATPTokenContract deployed to:`,
        satpTokenContract.target,
      );
      ...
      if (port === 8545) {
        ...
        const approve2Tx = await satpTokenContract
          .connect(user)
          .approve(BRIDGE_ADDRESS, 100);
      ...
      if (port === 8546) {
        const deployedSatpTokenContract = new ethers.Contract(
        ...
        console.log(`${port} - Approving bridge address ${BRIDGE_ADDRESS}...`);
        const approve2Tx = await deployedSatpTokenContract
          .connect(user)
          .approve(BRIDGE_ADDRESS, 100);
        await approve2Tx.wait();
    ...
    console.error(
      "Please provide the step as a number argument at call time (1, 2, or 3).",
    );
  }
}
