...
const SATP_TOKEN_BYTECODE =
  require("../artifacts/contracts/SATPNonFungibleTokenContract.sol/SATPNonFungibleTokenContract.json")[
    "bytecode"
  ];
...
async function main(port) {
  ...
  const SATPNonFungibleTokenContractFactory = new ethers.ContractFactory(
    SATP_TOKEN_ABI,
    SATP_TOKEN_BYTECODE,
    deployer,
  );
  const satpNonFungibleTokenContract =
    await SATPNonFungibleTokenContractFactory.deploy(deployerAddress);
  await satpNonFungibleTokenContract.waitForDeployment();
  console.log(
    `${port} - SATPNonFungibleTokenContract deployed to:`,
    satpNonFungibleTokenContract.target,
  );
  ...
  const giveRole2Tx = await satpNonFungibleTokenContract
    .connect(deployer)
    .grantBridgeRole(BRIDGE_ADDRESS);
  await giveRole2Tx.wait();

  if (port === 8545) {
    // Mint tokens to the user address in the source chain (8545)
    console.log(`${port} - Minting tokens...`);
    const mintTx = await satpNonFungibleTokenContract
      .connect(deployer)
      .mint(userAddress, 1001);
    await mintTx.wait();
    ...
    const approve2Tx = await satpNonFungibleTokenContract
      .connect(user)
      .approve(BRIDGE_ADDRESS, 1001);
