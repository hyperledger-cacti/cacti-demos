...
describe("SATPNonFungibleTokenContract", function () {
  async function deploySATPNonFungibleTokenContract() {
    ...
    SATPNonFungibleToken = await ethers.getContractFactory(
      "SATPNonFungibleTokenContract",
    );
    satp = await SATPNonFungibleToken.connect(deployer).deploy(
      deployer.address,
    );
  ...
  it("should initialize correctly with correct roles and ID", async function () {
    const { satp, deployer } = await loadFixture(
      deploySATPNonFungibleTokenContract,
    );
    expect(await satp.hasRole(await satp.OWNER_ROLE(), deployer.address)).to.be
      .true;
    expect(await satp.hasRole(await satp.BRIDGE_ROLE(), deployer.address)).to.be
      .true;
  });

  it("should allow minting by BRIDGE_ROLE", async function () {
    const { satp, deployer, user } = await loadFixture(
      deploySATPNonFungibleTokenContract,
    );
  ...
  it("should prevent minting by non-bridge address", async function () {
    const { satp, user } = await loadFixture(
      deploySATPNonFungibleTokenContract,
    );
  ...
  it("should fail assigning tokens if sender is not 'from'", async function () {
    const { satp, deployer, user, another } = await loadFixture(
      deploySATPNonFungibleTokenContract,
    );
    await satp.connect(deployer).mint(user.address, 1001);

    // user tries to assign tokens from `user` to another
    await expect(satp.connect(deployer).assign(another.address, 1001)).to.be
      .reverted;
  });
  ...
  it("should allow giving BRIDGE_ROLE to another address", async function () {
    ...
    expect(await satp.hasRole(await satp.BRIDGE_ROLE(), user.address)).to.be
      .true;
  });
  ...
  it("should revert hasBridgeRole if no permission", async function () {
    ...
    await expect(
      satp.connect(user).hasBridgeRole(user.address),
    ).to.be.revertedWithCustomError(satp, "noPermission");
  });
  ...
  it("should confirm permission if has BRIDGE_ROLE", async function () {
    ...
    await expect(
      satp.connect(deployer).hasBridgeRole(deployer.address),
